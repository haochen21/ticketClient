...
router.get('/', function (req, res) {
  ...
  //url = config.url + '/weixin/merchant/callback';
  // 重定向请求到微信服务器
  res.redirect(url)
})
  ...
  client.getAccessToken(code, function (err, result) {
    ...
    securityService.findUserByOpenId(openid, function (err, user) {
      ...
      if (err || user === null ) {
        ...
        client.getUser(openid, function (err, result) {
          ...
          securityService.createCustomerByWeixin(_user, function (err, result) {
            if (err) {
              ...
              console.log('merchant save sucess ....');
              console.log(result);
              req.session.auth = false;
              req.session.user = void 0;
              res.redirect('/?type=M&verifiy=true');
            }
          });
        ...
        // if phone_number exist,go home page
        if (user.phone) {
          res.redirect('/?type='+user.type+'&verifiy=true');
        } else {
          res.redirect('/?type='+user.type+'&verifiy=false');
        }
      }
    });
  });
